...
,(SELECT sum(office.price) FROM office WHERE DATE(office.created_at) = DATE(NOW()) AND office.system_user_id = syu.id) as office_cash_today
,(SELECT sum(office.price) FROM office WHERE DATE(office.created_at) BETWEEN DATE_FORMAT(NOW(), "%Y-%m-01") and DATE(NOW()) AND office.system_user_id = syu.id) as office_cash_month
,(SELECT sum(office.price) FROM office WHERE DATE(office.created_at) BETWEEN DATE_FORMAT(NOW(), "%Y-01-01") and DATE_FORMAT(NOW(), "%Y-12-31") AND office.system_user_id = syu.id) as office_cash_year
